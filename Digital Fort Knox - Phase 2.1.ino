...
void scanNetwork() {
  ...
  totalScans++;
  Serial.println();
  Serial.println("✓ Scan complete! Found " + String(found) + " devices");
...
void handleRoot() {
  ...
  html += "<h1>🛡️ Digital Fort Knox<div class='subtitle'>Security Dashboard<span class='badge'>v2.1 Hotspot</span></div></h1>";
  ...
  html += "</div>";
  html += "<button onclick='fetch(\"/scan\").then(()=>{alert(\"Scanning network...\");setTimeout(()=>location.reload(),15000)})'>🔍 Scan Now</button>";
  html += "</div>";
  
  // Devices
  html += "<div class='card'>";
  html += "<h2>🖥️ Monitored Devices</h2>";
  
  unsigned long currentTime = millis();
  
  // Trusted devices
  html += "<h3 style='color:#7ee787;margin:20px 0 10px 0'>✓ Trusted Devices</h3>";
  ...
  // Untrusted devices
  html += "<h3 style='color:#f85149;margin:20px 0 10px 0'>⚠️ Untrusted Devices</h3>";
  ...
  if (!hasUntrusted) {
    html += "<p style='color:#8b949e;padding:20px'>✓ All devices are trusted!</p>";
  }
  
  html += "</div>";
  
  html += "<div style='text-align:center;margin:20px 0;color:#8b949e;font-size:0.9em'>";
  html += "🔒 Digital Fort Knox v2.1 | Auto-refresh: 15s | Hotspot-Compatible Mode";
  html += "</div>";
